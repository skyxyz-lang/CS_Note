...
求二叉树的深度可以如下思路 
1. 定义递归函数含义为求root节点的深度 
2. 函数的递推表达式为 root节点深度=左叶子深度+右叶子深度 
3. 函数的出口为空节点深度为0

遍历二叉树的递归思路如下
1. 定义递归函数的含义为 遍历root为根的树 
2. 函数的递推表达式为 遍历root = 访问root + 遍历左子树 + 遍历右子树
3. 函数的出口为空节点直接返回

递归一般有几种
1. 一种是汉诺塔形式，直接定义求解函数，进行分解求值
2. 另外一种是二叉树的遍历的变种，定义遍历的递归函数，并且在遍历路径中记录状态进行求值。
#### 自顶向下（前序）
